...
template<int K>
__device__ void updateKBest_idx(const float3& ref, const float3& point, float* knn, 
const int idx, int* knn_idx)
{
	float3 d = { point.x - ref.x, point.y - ref.y, point.z - ref.z };
	float dist = d.x * d.x + d.y * d.y + d.z * d.z;
	int idx_copy = idx;
...
__global__ void boxMeanDist(uint32_t P, float3* points, uint32_t* indices, MinMax* boxes, float* dists, int* knn_indices)
{
	...
	int best_idx[3] = { INT_MAX, INT_MAX, INT_MAX };
	...
	for (int i = max(0, idx - 3); i <= min(P - 1, idx + 3); i++)
	{
		...
		updateKBest_idx<3>(point, points[indices[i]], best, indices[i], best_idx);
	}
	...
	knn_indices[indices[idx] * 3 + 0] = best_idx[0];
	knn_indices[indices[idx] * 3 + 1] = best_idx[1];
	knn_indices[indices[idx] * 3 + 2] = best_idx[2];
}

void SimpleKNN::knn(int P, float3* points, float* meanDists, int* knn_indices)
{
	float3* result;
	...
	boxMeanDist << <num_boxes, BOX_SIZE >> > (P, points, indices_sorted.data().get(), boxes.data().get(), meanDists, knn_indices);
